...
1 TCP/IP，一个进程通常有多个端口（其中一个用来通信）
2 命名管道和共享内存，服务器和客户端在同一台windows主机上
3 UNIX 域套接字，服务器和客户端在同一台UNIX主机上

客户端和服务器之间交互过程
1. 连接管理
2. 查询缓存（8.0后被删除），语法解析与查询优化
3. 存储引擎
...
1. 知道编码方式，在GB2312编码中，高位为0，意味着一个字节代表一个字符；为1，两个字节代表一个字符



ASCII：128个常用字符，占用一个字节

ISO 8859-1（Latin1）：在ASCII 上扩展了128个西欧常用字符

utf8mb3，阉割版的UTF-8, 只用1~3个字节
utf8mb4    标准的UTF-8，占用4个字节

一个字符拥有多种比较规则，如是否区分大小写

MySQL 有 4个级别的字符集和比较规则，分别是服务器级别，数据库级别，表级别，列级别
...
chcp 65001
将字符集临时切换为 UTF-8（65001），但仅对当前命令窗口有效。
```
...
1. COMPACT：溢出列会记录部分数据和指向溢出页的地址

2. REDUNDANT， before 5.0, 

3. DYNAMIC，5.7版本默认，相比于COMPACT格式，DYNAMIC溢出列只记录溢出页的地址，

4. COMPRESSED：对页进行压缩

  如果一个列的数据长度非常大（溢出列， off-page 列），记录只存储该列的一部分数据+溢出页的地址，把剩下的数据存储大其他页中（溢出页）
...
| record_type  | 3          | 0：普通记录，1：B+数非叶子节点的**目录项记录**，索引，目录项记录只有**主键值和页号**两列，2：Infimum记录，3：Supremum记录。 |
| next_record  |            | 表示从当前记录的**真实数据**到下一条记录的真实数据的**距离（偏移量）**。Infimum记录的下一条记录是主键值最小的记录，Supremum记录的上一条记录时主键值最大的记录。记录间通过next_record字段组成了一个按照主键升序的单向链表。 |
...
| Infimum+Supremum |          | 虚拟记录，页中最小记录和页中最大记录                         |
...
1. InnoDB会将页中的记录分组，每个组中的最后一条记录的地址偏移量作为一个槽，记录到Page Directory中
2. 通过二分法确定该记录所在分组对应的slot，并找到该槽所在分组中主键值最小的记录。slot之间在物理空间上相连
3. 通过记录中的next_record 属性遍历该槽所在的组中的各个记录。
4. 记录之间通过next_record形成一个按照主键值升序的单向链表
...
- 非叶子节点（内节点）：主要是索引页，存放目录项记录（主键+页编号）的节点
- 叶子节点：主要是数据页，存放用户所有数据

InnoDB 索引分类：

- 聚簇索引：以**主键值的大小作为页和记录的排序规则**，叶子节点存放记录的所有列，所以 "索引即数据,数据即索引”。非叶子节点中的目录项记录保存（页中最小主键值和页编号）
- 二级索引（辅助索引）：**以非主键列（一个）的大小作为页和记录的排序规则**，叶子节点存放只记录的**索引列和主键值**，非叶子节点中的目录项记录保存**（索引列值+页中最小主键值+页编号）**。**如果查找时查找非索引列的值，需要根据主键信息到聚簇索引中查找完整的用户信息**。该操作称为回表查询。
- 联合索引：多个列的大小作为页和记录的排序规则。idx_c2_c3,  记录先按照c2 排序，如果c2相同，按照c3排序，c3相同，按照主键排序

**主键值的大小作为页和记录的排序规则**

> 1. 页中的记录按照主键大小递增构成一个单向列表
> 2. 数据页按照主键大小构成一个双向列表
> 3. 索引页：同一层级中的页按照目录项记录中的主键构成一个双向列表
...
访问方法：执行查询语句的方式，使用哪种方式查询记录由优化器决定
...
内连接：针对驱动表中的某条记录，如果在被驱动表中没有找到匹配的记录，则不将该驱动表的记录加入到结果集中

外连接：针对驱动表中的某条记录，如果在被驱动表中没有找到匹配的记录，仍然将该驱动表的记录加入到结果集中

左外连接：选取左侧的表作为驱动表

where子句过滤：对结果集进行筛选。不符合where子句中的过滤条件的记录都不会加入到最终的结果集中

on子句过滤：对于外连接驱动表中的记录，如果在被驱动表中找不到符合on子句中过滤条件的记录，驱动表中的记录会加入到结果集中，但是被驱动表记录的各个字段使用null值填充

> 专门为 "外连接驱动表中的记录在被驱动表中匹配不到记录时，是否应该将该驱动表中的记录加入到结果集中" 提出，所以在内连接中where 和on等价

### 查询重写（第十四章 todo）

MySQL 会根据一些规则自动重写用户的SQL
...
EXPLAIN SQL
每一张表都对应执行计划中的一条记录
1. id：标识SQL，相同id，从上往下，顺序执行，上边的为驱动表，小表先执行（小表驱动大表）
2. select_type
    - simple 简单查询，不包含子查询和 union
    - primary 包含子查询 SQL 中的主查询（最外层）
    - union：second or later SELECT statement in a UNION
    - union result：result of a UNION    
    - SUBQUERY：First SELECT in subquery,不相关子查询，包含子查询 SQL 中的子查询（非最外层）
    - DEPENDENT SUBQUERY：First SELECT in subquery, depend on outer query 相关子查询
    - DEPENDENT UNION: second or later SELECT statement in a UNION, depend on outer query
...
6. key_len：索引字段的总长度
     主要在使用联合索引时用，用于判断那几个列的搜索条件充当扫描区间的边界条件
    utf8 中，1 个字符占 3 个字节
    gbk 中，1 个字符 2 个字节
    latin 中，1 个字符 1 个字节
    ...
    key_part_1定义为 varchar(100)  , utf8字符集
    where key_part_1 = 'a' 
    key_len = 100*3+1（允许为null）+ 2 (变长类型列)
 7. ref：索引列匹配的东西，常量，函数
    select ... where a.c=b.x
    其中 b.x 可以是常量，const
 8. rows：全表扫描时，该表的预估行数。索引查询时，索引记录行数
9. Extra 
       Using filesort
       Using temporary
       Using index
...
redo log 为了系统崩溃重启时，将数据库恢复到崩溃前的状态，防止修改丢失

redo log 记录了 事务执行过程中都修改了哪些内容



数据写到磁盘的过程

data --->  redo log ---> redo log buffer----> disk
...
- 只读事务：不可以对普通表进行修改和删除，但是可以对临时表进行修改和删除 `TART TRACTION READ ONLY`
- 读写事务：可以对普通表进行修改和删除
...
trx_id 事务ID：本质是一个递增数字，后分配事务的id大于先分配的事务id，只有在对表进行增删改时才分配事务ID。未开启事务的trx_id 为 0.



事务的原子性通过redo log 和 undo log 保证。
...
脏写：T1 修改了 T2 中未提交的修改数据

脏读：T1读取了T2中未提交的修改数据

不可重复读：T1读取了T2中提交的修改数据
...
| 隔离级别         | 脏写   | 脏读   | 不可重复读 | 幻读   | 标识 |
| ---------------- | ------ | ------ | ---------- | ------ | ---- |
| READ UNCOMMITTED | 不可能 | 可能   | 可能       | 可能   | R1   |
| READ COMMITTED   | 不可能 | 不可能 | 可能       | 可能   | R2   |
| REPEATABLE READ  | 不可能 | 不可能 | 不可能     | 可能   | R3   |
| SERIALIZABLE     | 不可能 | 不可能 | 不可能     | 不可能 | R4   |

MVCC（Multi-Version Concurrency Control）：多版本并发控制, 通过ReadView （一致性视图）和版本链实现，根据ReadView 查找小于min_trx_id 的版本

版本链：记录每修改一次都会生成一条undo log ，记录和undo log 之间通过roll_pointer 按照事务id 大小递减构成一条链表。版本链的头节点为当前记录的最新值。

ReadView ：执行select语句，用以判断当前哪些版本对当前事务可见，其结构主要如下

- m_ids: 生成ReadView 时，系统中活跃事务的事务id列表, 代表事务未提交
- min_trx_id：min(m_ids)
- max_trx_id：分给下一个事务的trx_id
- create_trx_id：生成该ReadView 的trx_id

找到 小于 min_trx_id 的版本，该版本事务已提交，对当前查询可见

事务各隔离级别的实现原理

1. R1: 直接读取最新版本的值
2. R2: 每次select 时生成一个ReadView
3. R3: 只在第一次select 时生成一个ReadView
4. R4: 加锁
...
事务在并发时（写-写，读-写，写-读）可能引发一致性问题，  通过锁来让事务排队执行，从而解决一致性问题。锁的本质是内存中的结构，其最重要的属性如下
...
S，共享锁：读取(select)一条记录时，需要先获取该记录的S锁

X，排他锁：修改(delete, update)一条记录时，需要先获取该记录的X锁

只要被加了X锁，则不能再在该记录上获取X锁或S锁

只要被加了S锁，则不能再在该记录上获取X锁，但是仍可以获取S锁



获取S锁：`select ... lock in share mode`

获取X锁：`select ... for update`

S 和 X 都可以在表级和行级别上加锁



IS与IX表级锁的提出主要是为了在表上加S锁和X锁时可以快速判断表中的记录是否被上锁，所以IX 与 IS 之间相互兼容。

- IS，意向共享锁，如果在记录上加S锁，先在表级别加IS锁
- IX，意向独占锁，如果在记录上加X锁，先在表级别加IX锁
...
- Next-Key Lock: Record Lock 和 Gap Lock 的结合体
- 隐式锁：依靠记录中的trx_id 属性保护不被别的事务改动该记录
...
普通select 语句：在各个事务隔离级别都不加锁。通过MVCC 保证一致性
...
对R1， R2 下加Record Lock，R3，R4 下加next_key lock

R1， R2

1. 查找扫描区间的第一条记录
2. 对当前记录加锁
3. 如果是二级索引，需要进行回表操作，对聚簇索引也要加锁
4. 判断边界条件是否成立
   - 成立，跳到下个步骤
   - 不成立，释放该记录上的锁（R1， R2）
5. server 层判断其余搜索条件是否成立
   - 成立，将记录发送客户端
   - 不成立，释放该记录上的锁（R1， R2）
6. 获取下一条记录，跳回步骤2

加锁是一条一条记录加锁，先加锁，若不符合搜索条件，再释放锁

死锁：不同事务以不同的顺序对记录加锁，可能会发生死锁。死锁发生时，InnoDB会选择一个较小的事务进行回滚