...
final修饰变量（成员变量或局部变量）时称为常量，名称大写， 且只能被赋值一次。变量中的**对象引用不可变**，但是这个对象的内容可以更改。

final标记的成员变量必须在声明时或在每个构造器中或代码块中显式赋值， 然后才能使用。final double MY_PI = 3.14;

final修饰类时，该类不能被继承，类中的方法不会被覆盖，默认都是final, 如String类、 System类、 StringBuffer类

final修饰方法时，该**方法可以被继承**，**但是不能被覆盖override**， Object类中的getClass()
