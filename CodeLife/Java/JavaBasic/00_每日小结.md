...
底层是`private final char value[]`因为其使用final修饰符，所以他的引用不会变，又因为其声明为private, 只能在当前类中进行访问，而当前类并没有对value进行改变。虽然子类不能访问父类的private变量，但是其可以通过其它方法来修改（set, get），所以将String声明为`final class String`, 防止继承破坏了其不可变性。
