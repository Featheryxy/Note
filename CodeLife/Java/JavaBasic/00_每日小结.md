...
MyClass.class.isAssignableFrom(Other.class) will return true if MyClass is the same as, or a superclass or superinterface of, Other
...
2. 读取配置文件&注册Bean，读取xml文件并解析，构建beanDefinition 并缓存到 beanDefinitionMap
3. 创建bean，根据beanName 和 对应的 beanDefinition 创建bean，属性填充，并缓存到 singletonObjects
