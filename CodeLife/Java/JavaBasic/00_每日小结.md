...
语义
Bean定义：BeanDefinition, 存储了Class
beanDefinitionMap: map, 用于缓存 beanDefinition
singletonObjects：map，bean容器，用于存放bean

Bean对象：创建，注册，获取
Bean容器：hashMap，存储Bean
Bean工厂：BeanFactory，获取bean对象，如果对象在 用于创建Bean
- 缓存Bean
- 单例
...
SingletonBeanRegistry：接口，提供单例bean的注册，获取功能
DefaultSingletonBeanRegistry：SingletonBeanRegistry的实现类
- singletonObjects：map，bean容器，用于存储bean

BeanFactory: 接口，提供bean的获取功能
AbstractBeanFactory：抽象类，BeanFactory的实现类。
...
AbstractAutowireCapableBeanFactory： AbstractBeanFactory的子类
定义了 createBean 的抽象模板方法，并将其注册到容器中

BeanDefinitionRegistry：接口，提供beanDefinition的注册功能。
DefaultListableBeanFactory：BeanDefinitionRegistry的实现类，AbstractAutowireCapableBeanFactory的子类。
提供 beanDefinition 的注册，获取功能
- beanDefinitionMap: map, 用于缓存 beanDefinition

Resource: 接口，资源，获取 InputStream
ResourceLoader: 接口，资源加载器

BeanDefinitionReader：接口，
AbstractBeanDefinitionReader: BeanDefinitionReader的实现类
XmlBeanDefinitionReader：继承AbstractBeanDefinitionReader，
读取，解析XML配置文件，创建BeanDefinition,并将其注册到容器中
...
>BeanPostProcessor 在BeanAbstractBeanDefinitionReader:
对象执行初始化方法前后，对Bean对象进行修改，记录，替换
...
Aware 接口：标记性接口，将容器中的对象注入到当前对象中。让实现的子类可以感知到容器中相关对象，如 ApplicationContext，BeanFactoryAware
